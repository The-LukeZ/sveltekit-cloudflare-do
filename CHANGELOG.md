...
- Replaced `tsc` with `tsdown` as the build tool (Rolldown-powered, faster builds, better ESM support, built-in type definitions, etc.)
  - tsdown also generates the exports in `package.json` automatically based on the entry points, so the `exports` field is now generated by tsdown instead of being manually maintained. It should not be modified manually anymore.
...
- Initial release of sveltekit-cloudflare-do (forked from sveltekit-cloudflare-durable-objects)
...
[0.1.0]: https://github.com/The-LukeZ/sveltekit-cloudflare-do/releases/tag/v0.1.0
